...
output_dir <- 'E:/Research Projects/Worker Accidents and Pollution/Data/Inversion data/Nighttime Inversions'
...
for (year in begin_year:end_year) {
  ...
  for (month in 1:12) {
    month_data <- read_dta(file.path(input_dir, input_file(year, month))) %>%
      ...
      # Filter by night/early morning inversions to avoid visibility concerns
      filter(hour(ti) <= 6) %>%
