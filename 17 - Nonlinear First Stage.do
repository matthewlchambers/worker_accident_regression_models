...
* Generate powers of inversion coverage as separate variables
forvalues i = 1/5 {
...
eststo: ivreghdfe accident_occurred mean_temperature mean_precipitation employment weekday_dummy_* (mean_pm25 = inversion_coverage_*), absorb(fips) cluster(fips) first
eststo: ivreghdfe accident_occurred mean_temperature mean_precipitation employment weekday_dummy_* (mean_pm25 = inversion_coverage_*), absorb(fips month) cluster(fips) first

* Save the stored regressions as latex and rtf tables, then clear them so I can save the next batch.
esttab using nonlinear_first_stage.tex, replace label mtitles booktabs alignment(D{.}{.}{-1}) title(Nonlinear First Stage\label{tab1})
esttab using nonlinear_first_stage.rtf, replace label mtitles nogap onecell
eststo clear
