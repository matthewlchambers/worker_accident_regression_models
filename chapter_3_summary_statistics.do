...
estpost tabstat accident_occurred mean_pm25 pbl_height *_lapse heat_degree_days cool_degree_days mean_precipitation wind_speed relative_humidity population, by(season) statistics(mean sd) columns(statistics)
esttab using chapter_3_summary_stats_season.tex, main(mean) aux(sd) nostar noobs unstack nonote replace label nomtitles booktabs alignment(D{.}{.}{-1}) title(Summary Statistics by Season\label{sumstats_season})
esttab using chapter_3_summary_stats_season.rtf, main(mean) aux(sd) nostar noobs unstack nonote replace label nomtitles nogap onecell
