...
* Generate injury rate and accident rate as alternative dependent variables
gen injury_rate = num_injured / employment
gen accident_rate = num_accidents / employment


* Do linear iv regressions
// eststo: ivreghdfe injury_rate mean_temperature mean_precipitation weekday_dummy_* (mean_pm25 = inversion_coverage), absorb(fips) cluster(fips) first
// eststo: ivreghdfe accident_rate mean_temperature mean_precipitation weekday_dummy_* (mean_pm25 = inversion_coverage), absorb(fips) cluster(fips) first
eststo: ivreghdfe injury_rate mean_temperature mean_precipitation weekday_dummy_* (mean_pm25 = inversion_coverage), absorb(fips#month) cluster(fips) first
eststo: ivreghdfe accident_rate mean_temperature mean_precipitation weekday_dummy_* (mean_pm25 = inversion_coverage), absorb(fips#month) cluster(fips) first

* Save the stored regressions as latex and rtf tables, then clear them so I can save the next batch.
esttab using inj_acc_rates.tex, replace label mtitles booktabs alignment(D{.}{.}{-1}) title(Injury and Accident Rates\label{tab1})
esttab using inh_acc_rates.rtf, replace label mtitles nogap onecell
eststo clear
