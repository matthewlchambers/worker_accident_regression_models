...
* Start logging
log using `industry'_second_stage_LASSO.log, replace name(main)
...
forvalues i = 1/5 {
	reghdfe mean_pm25_`i' meanlayer* pbl_height heat_degree_days cool_degree_days mean_precipitation employment, absorb(fips weekday month) cluster(fips)
	predict pm25_hat_`i', xb
}

* Note that these standard errors will be wrong, but I just want to see which variables are selected.
eststo: rlasso accident_occurred pm25_hat_* meanlayer* pbl_height heat_degree_days cool_degree_days mean_precipitation employment i.weekday i.month, sqrt fe cluster(fips)
